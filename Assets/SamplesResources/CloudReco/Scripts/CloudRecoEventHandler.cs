...
{
    private IMobileMessageWrapper mobileMessage = new MobileMessageWrapper();

    private IObjectsManager _objectsManager = new ObjectsManager();
        ...
        switch (updateError)
        {
            case TargetFinder.UpdateState.UPDATE_ERROR_AUTHORIZATION_FAILED:
                ShowError("Authorization Error",
                    "The cloud recognition service access keys are incorrect or have expired.");
                break;
            ...
            case TargetFinder.UpdateState.UPDATE_ERROR_REQUEST_TIMEOUT:
                ShowError("Request Timeout",
                    "The network request has timed out, please check your internet connection and try again.");
                break;
    ...
    public void OnNewSearchResult(TargetFinder.TargetSearchResult targetSearchResult)
    {
        ...
        try
        {
            var description = _objectsManager.GetObjectDesription(targetSearchResult.UniqueTargetId);
            GlobalItems.TrackableDescription = description;
            //mobileMessage.ShowMessage("AR", description);
        }
        catch (Exception exc)
        {
